...
EXECUTE sql_poly;	
create temporary table tmp_poly_result as select * from tmp_poly limit 1;
ALTER TABLE tmp_poly_result add column id serial not null;
truncate tmp_poly_result;
i:=0;

FOR geo in select geom from tmp_poly LOOP
i:=i+1;
SELECT st_area(geo.geom) / 250000  INTO n;
...
DROP TABLE IF EXISTS poly_divided;
	...
	sql_poly:= concat('CREATE TEMPORARY TABLE poly_pts AS
 					  SELECT (ST_Dump(ST_GeneratePoints(geom, 2000))).geom AS geom FROM tmp_poly');
